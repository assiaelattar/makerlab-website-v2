...
| --- | --- | --- | --- | --- |
| P0 | In progress | Publish the current public experience update | Approved frontend-only delta passes continuity and production builds, lands on `origin/main`, then changes are observed on `space.makerlab.academy` | Complete live verification |
| P0 | Completed | Establish a release ledger for local-to-live handoff | Every deployment records approved scope, branch, commit, Hostinger target, timestamp, and observed live result | Reusable `$hostinger-main-release` workflow installed |
