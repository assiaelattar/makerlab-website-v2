...
- The approved public frontend release was prepared from the latest `origin/main` on 2026-07-20 for direct promotion to Hostinger's watched `main` branch.
- Release scope includes the Home journey and video moments, program and mission pages, Store discovery and product detail UX, parent decision guidance, generated imagery, and SEO/LLM metadata.
- Admin routes, `App.tsx`, `components/AdminLayout.tsx`, APIs, Firebase rules/configuration, credentials, and the local Signal Room are explicitly outside this public release.
- The local `App.tsx` and `components/AdminLayout.tsx` modifications remain deliberately unstaged in the working project.
...
- GitHub CLI is authenticated as `assiaelattar`, and Hostinger deploys this project from `origin/main`. A feature-branch push is backup only, never deployment evidence.
