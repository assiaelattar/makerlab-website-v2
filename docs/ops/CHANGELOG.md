...
## 2026-07-20 - Public experience release prepared for Hostinger

- Rebuilt the approved public frontend delta on the latest `origin/main` instead of merging the long-lived feature branch.
- Included Home, program, mission, Store, parent-decision, video-gallery, generated-image, and SEO/LLM improvements.
- Explicitly excluded admin routes, local admin edits, APIs, Firebase deployment files, credentials, and the localhost-only Signal Room.
- Required both continuity and production-build validation before promotion to Hostinger's watched `main` branch.
