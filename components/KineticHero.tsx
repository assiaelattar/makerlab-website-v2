...
export const KineticHero: React.FC<KineticHeroProps> = ({ images1 = [], images2 = [], images3 = [] }) => {
  return (
    <div className="relative w-full grid grid-cols-2 md:flex md:flex-row gap-3 md:gap-6 perspective-1000">
      ...
      <div className="col-span-2 order-1 md:order-2 md:flex-[1.2] md:hover:flex-[1.8] transition-all duration-500 ease-in-out group relative h-[180px] md:h-full bg-brand-blue border-4 border-black rounded-2xl md:rounded-3xl overflow-hidden shadow-neo-lg hover:shadow-none hover:translate-x-1 hover:translate-y-1">
        ...
        <div className="h-full w-full p-3 md:p-6 flex flex-col justify-between relative z-10 text-white">
          <div className="flex justify-between items-start">
            <div className="bg-white border-2 border-black p-2 md:p-3 rounded-xl md:rounded-2xl shadow-neo-sm group-hover:-rotate-12 transition-transform">
              <Terminal className="text-black" size={16} strokeWidth={3} />
            </div>
          ...
          <div className="flex-1 my-2 md:my-6 flex flex-col justify-center gap-2 md:gap-4 overflow-hidden">
            <KineticBelt images={images2} direction="horizontal" speed="12s" className="h-16 md:h-32" />
            <KineticBelt images={images3} direction="horizontal" speed="18s" reverse className="h-16 md:h-32" />
          </div>
          
          <div className="bg-black border-2 md:border-4 border-white p-2 md:p-4 shadow-[4px_4px_0px_0px_white] self-start">
            <h4 className="font-display font-black text-xs md:text-2xl leading-none uppercase italic underline decoration-brand-green underline-offset-2 md:underline-offset-4">Code Your World.</h4>
          </div>
        </div>
      </div>

      {/* ZONE 1: ROBOTICS (Bottom Left on Mobile) */}
      <div className="col-span-1 order-2 md:order-1 md:flex-[1] md:hover:flex-[1.5] transition-all duration-500 ease-in-out group relative h-[140px] md:h-full bg-brand-red border-4 border-black rounded-2xl md:rounded-3xl overflow-hidden shadow-neo-lg hover:shadow-none hover:translate-x-1 hover:translate-y-1">
      ...
      <div className="col-span-1 order-3 md:order-3 md:flex-[1] md:hover:flex-[1.5] transition-all duration-500 ease-in-out group relative h-[140px] md:h-full bg-brand-orange border-4 border-black rounded-2xl md:rounded-3xl overflow-hidden shadow-neo-lg hover:shadow-none hover:translate-x-1 hover:translate-y-1">
      ...
      {/* Floating Interactive Badge — desktop only, causes overlap on mobile */}
      <div className="hidden md:flex absolute md:top-[-40px] md:left-[-40px] bg-white text-black border-4 border-black px-6 py-4 rounded-none transform md:-rotate-12 shadow-neo-xl hover:shadow-none transition-all cursor-pointer z-30 items-center gap-4 animate-float group/ticket">
