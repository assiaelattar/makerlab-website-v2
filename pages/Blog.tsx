...
export const Blog: React.FC = () => {
  ...
  // Use dynamic posts from settings if available, otherwise fallback to mock
  ...
  return (
    <div className="min-h-screen py-20 px-4">
      <SEO 
        ...
        description="Actualités, conseils et projets tech pour enfants et parents. Découvrez nos derniers articles sur la robotique, le codage et l'innovation."
        keywords="blog robotique Maroc, actualités STEM enfants, conseils parents tech, MakerLab Academy blog"
      />
            ...
            <ScrollReveal key={post.id} delay={index * 100}>
              <article className="bg-white rounded-3xl border-4 border-black shadow-neo overflow-hidden transform hover:-translate-y-2 transition-transform duration-300">
                <div className="grid grid-cols-1 md:grid-cols-2">
                  <Link to={`/blog/${post.slug || post.id}`} className="h-64 md:h-auto border-b-4 md:border-b-0 md:border-r-4 border-black relative block overflow-hidden">
                    <img
                      src={post.image}
                      alt={post.title}
                  ...
                  <div className="p-8 flex flex-col justify-center">
                    ...
                    <Link to={`/blog/${post.slug || post.id}`}>
                      <h2 className="font-display font-black text-2xl mb-4 hover:text-brand-red cursor-pointer transition-colors leading-tight uppercase tracking-tight">{post.title}</h2>
                    </Link>
